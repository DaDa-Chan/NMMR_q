...
class NMMR_H_Trainer:
    pass
    def __init__(self, data_configs: Dict[str, Any], train_params: Dict[str, Any], random_seed: int, dump_folder: Optional[Path]=None):
        ...
        self.h_loss = NMMR_H_Loss(use_u_statistic=self.train_params.get('use_u_statistic', False))
        self.writer = None
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader, verbose: int=0) -> MLP_for_NMMR:
        ...
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=sched_factor, patience=sched_patience, min_lr=1e-06)
        patience = self.train_params.get('patience', 20)
        early_stopping = EarlyStopping(patience=patience, min_delta=1e-05)
